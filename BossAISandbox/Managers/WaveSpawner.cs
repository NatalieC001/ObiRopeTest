...
{
    [Header("Dependencies")]
    public LevelProgressionManager progressionManager; // Assumed external dependencies not fully defined in sandbox but kept for structure
    ...
    public int activeEnemyCount = 0; // Made public for external tools to see
    ...
    public GameObject SpawnBossMinion(Vector3 position, Quaternion rotation)
    {
        ...
        int idx = UnityEngine.Random.Range(0, bossMinionRoster.Count);
        ...
        if (prefab != null)
        {
            ...
            // If the wave is active, we track it just like a regular spawned minion
            if (isWaveActive)
            {
                activeEnemyCount++;
            }
